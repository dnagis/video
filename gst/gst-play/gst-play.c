...
play_timeout (gpointer user_data)
{
  GstPlay *play = user_data;
  gint64 pos = -1, dur = -1;
  int vvnx_pos_s = -1; //la position en secondes
  ...
  if (pos >= 0 && dur > 0) {
	...
	vvnx_pos_s = GST_TIME_AS_SECONDS(pos);   
    
    gst_print (" %s / %s vvnx_seconds: %i %s\r", pstr, dstr, vvnx_pos_s, status); 
  }
...
main (int argc, char **argv)
{
  ...
  /* prepare */
  play = play_new (uris, audio_sink, "vaapisink", gapless, volume, verbose,
      flags, use_playbin3);
