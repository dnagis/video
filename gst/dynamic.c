/**
 * gcc dynamic.c -o dynamic `pkg-config --cflags --libs gstreamer-1.0`
 * 
 * Objectif: avoir l'équivalent de:
 * gst-launch-1.0 filesrc location=file.mp4 ! decodebin ! videoconvert ! ximagesink
 * 
 * Basé sur le tuto dynamic pipeline (basic-tutorial-3):
 * https://gstreamer.freedesktop.org/documentation/tutorials/basic/dynamic-pipelines.html?gi-language=c
 * 
 * Le crux = link impossible tant qu'un pad n'existe pas:
...
typedef struct _CustomData
{
  GstElement *pipeline;
  GstElement *source;
  GstElement *decode; //resample (du tuto basic n'existe plus 
  GstElement *convert;
  GstElement *sink;
} CustomData;
...
main (int argc, char *argv[])
{
  ...
  /* Build the pipeline. Note that we are NOT linking decodebin at this point. We will do it later. */
  ...
  /* Set the URI to play */
  //g_object_set (data.source, "location", "/root/capture-1.mp4", NULL);
